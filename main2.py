...
# todo add new types of groups
# todo add macro trackers and R tracker
# todo add multi processing

# todo start if vaccination at different moment
...
class track_statistics(object):
    ...
    def output(self, N, timesteps):
        import csv

        name = str(N) + ", " + str(timesteps)
        data = [[self.susceptible], [self.infectious], [self.symptomatic], [self.quarantined], [self.hospitalised], [self.recovered], [self.vaccinated], [self.transmitter], [self.deceased]]
        f = open(name, 'w')
...
def create_people(groups, start_age, nages, N, vaccination_readiness):
    new_groups = groups
    people = []
    for age in nages[:-1]:  # add people of all but highest age
...
def create_network(groups, degrees):
    ...
    for gi in range(ngroups):
        ...
        for gj in range(ngroups):
            ...
            out += create_subnetwork(Groepen[gi], Groepen[gj], degrees[gi][gj], i0, j0)
            print("procent: ", teller)
            teller+=1
...
def vaccinate(people, status_changes, order=0):
    ...
    """
    # todo make this exlusive and thereby faster
    new_status_changes = status_changes
    n = len(people)
...
def initialize_model(N, BETA, vaccination_readiness):
    ...
    adults = sum([k for k in population])
    ...
    status_changes_init_2 = initialise_infection(people, P0, status_changes_init_0)
    print("Infection initialized")
    #status_changes_init_2 = initialise_vaccination(N, people, BETA0, VACC0, status_changes_init_1)
    print("Vaccination initialized")
...
def run_model(timesteps, contact_matrix, people, fatality_distribution, tracker):
    for time in range(timesteps):
        status_changes_0 = {"susceptible": 0,
                            "quarantined": 0,
                            "symptomatic": 0,
                            "hospitalized": 0,
                            "recovered": 0,
        ...
        if time < 100:
            ...
            tracker.update_all(status_changes_2)
        else:
            status_changes_1 = infect(contact_matrix, people, status_changes_0)
            status_changes_2 = update(fatality_distribution, people, status_changes_1)
...
print("initialization takes:", end_init - start_init)
print("model takes:", end_model - start_model)
tracker_2.output(N, timesteps)
